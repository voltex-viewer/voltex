...
export class WaveformRenderObject extends RenderObject {
    ...
    private sharedMiterJoinGeometryBuffer: WebGLBuffer;
    ...
    constructor(config: WaveformConfig, bufferData: ChannelBufferData, sharedInstanceGeometryBuffer: WebGLBuffer, sharedMiterJoinGeometryBuffer: WebGLBuffer, instancingExt: ANGLE_instanced_arrays, color: string, waveformPrograms: WaveformShaders, signal: Signal, zIndex: number = 0) {
        super(zIndex);
        this.config = config;
        this.bufferData = bufferData;
        this.sharedInstanceGeometryBuffer = sharedInstanceGeometryBuffer;
        this.sharedMiterJoinGeometryBuffer = sharedMiterJoinGeometryBuffer;
    ...
    render(context: RenderContext, bounds: RenderBounds): boolean {
        ...
        if (renderMode === RenderMode.Lines) {
            ...
            this.renderMiterJoins(gl, this.waveformPrograms.miterJoin, linesBindUniforms);
    ...
    private renderMiterJoins(
        ...
        // Skip miter joins for discrete signals as they use horizontal lines
        ...
        // Bind miter join geometry (6 vertices for 2 triangles)
        gl.bindBuffer(gl.ARRAY_BUFFER, this.sharedMiterJoinGeometryBuffer);
        const positionLocation = gl.getAttribLocation(program, 'position');
        gl.enableVertexAttribArray(positionLocation);
        ...
        // Draw instanced miter joins - need 3 consecutive points
